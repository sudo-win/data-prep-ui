...
const submitForm = (event) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setSampleRecords(JSON.parse(response.data.replace("'sample_records'","\"sample_records\"")
             .replaceAll("'{","{")
             .replaceAll("}'","}")
             .replace("'shape'","\"shape\"")
             .replace("'dtypes'","\"dtypes\"")
             .replace("'non_null_count'","\"non_null_count\""))['sample_records']);
        
        setShape(JSON.parse(response.data.replace("'sample_records'","\"sample_records\"")
        .replaceAll("'{","{")
        .replaceAll("}'","}")
        .replace("'shape'","\"shape\"")
        .replace("'dtypes'","\"dtypes\"")
        .replace("'non_null_count'","\"non_null_count\""))['shape']);

        setDtypes(JSON.parse(response.data.replace("'sample_records'","\"sample_records\"")
        .replaceAll("'{","{")
        .replaceAll("}'","}")
        .replace("'shape'","\"shape\"")
        .replace("'dtypes'","\"dtypes\"")
        .replace("'non_null_count'","\"non_null_count\""))['dtypes']);

        setNullCount(JSON.parse(response.data.replace("'sample_records'","\"sample_records\"")
        .replaceAll("'{","{")
        .replaceAll("}'","}")
        .replace("'shape'","\"shape\"")
        .replace("'dtypes'","\"dtypes\"")
        .replace("'non_null_count'","\"non_null_count\""))['non_null_count']);
    ...
    { state && 
      ...
      <div className="form">
          ...
          <form onSubmit={submitForm}>
            <br />
            <input type="file" className="btn-file-upload" onChange={onChangeMethod} />
            <br />
            <br />
            <button type="submit" className="btn btn-primary btn-sm">Upload File</button>
            </form>
           <br />   
           <div className="progress-bar"><ProgressBar now={progress} /></div> 
